<?php
class concomite{
    const NOMBRE_TABLA = "concepto_comite";
    //Numero de serie
    const CONCOM_ID = "concom_id";
    const CONCOM_DESCRIPCION = "concom_descripcion";
    const FAM_ID = "fam_id";
    const CONCOM_PARTIDA_GLOBAL = "concom_partida_global";
    const CONCOM_PARTIDA_ESPECIFICA = "concom_partida_especifica";
    const CONCOM_ACTIVO = "concom_activo";
    const CONCOM_CREADO_POR = "concom_creado_por";
    const CONCOM_CREADO_EL = "concom_creado_el";
    const CONCOM_MODIFICADO_POR = "concom_modificado_por";
    const CONCOM_MODIFICADO_EL = "concom_modificado_el";
    ...
    const CAMPOS = array(
        self::CONCOM_ID,
        self::CONCOM_DESCRIPCION,
        self::FAM_ID,
        self::CONCOM_PARTIDA_GLOBAL,
        self::CONCOM_PARTIDA_ESPECIFICA,
        self::CONCOM_ACTIVO
    );

    const SELECT = "select cc.concom_id, cc.concom_descripcion,
    cc.fam_id, f.tipo_descripcion as Familia,
    cc.concom_partida_global, cc.concom_partida_especifica,
    cc.concom_activo,
    cc.concom_creado_por, cc.concom_creado_el,
    cc.concom_modificado_por, cc.concom_modificado_el
    from ";

    const INNER = " cc left join tipo f on cc.fam_id = f.tipo_clave and f.clatip_id = 'FAMPROD' ";
    ////////////////////////////////////////////////////////////////////////////////////////////////
    //METODOS
    ////////////////////////////////////////////////////////////////////////////////////////////////
    /**
    ...
    {
        try {
            $whereId = !empty($id) ? " WHERE " . self::VEH_ID . "= :concom_id" : "";

            //$comando = "SELECT * FROM " . self::NOMBRE_TABLA . $whereId;
            $comando = self::SELECT . self::NOMBRE_TABLA . self::INNER . $whereId . " order by concom_id";
            $sentencia = ConexionBD::obtenerInstancia()->obtenerBD()->prepare($comando);

            if (!empty($id)) {
                $sentencia->bindParam(":concom_id", $id, PDO::PARAM_INT);
            }
    ...
    /**
     * Crea un concepto de comite en la base de datos
     *
    ...
    private static function crear($idUsuario)
    {
        ...
        if ($datos) {
            ...
            $concomDescripcion = htmlspecialchars(strip_tags($datos->concom_descripcion));
            $famId = htmlspecialchars(strip_tags($datos->fam_id));
            $concomPartidaGlobal = htmlspecialchars(strip_tags($datos->concom_partida_global));
            $concomPartidaEspecifica = htmlspecialchars(strip_tags($datos->concom_partida_especifica));
            ...
            try {
                ...
                $comando = "INSERT INTO " . self::NOMBRE_TABLA . " ( " .
                    ...
                    self::CONCOM_PARTIDA_GLOBAL . ", " .
                    self::CONCOM_PARTIDA_ESPECIFICA . ", " .
                    self::CONCOM_CREADO_EL . ", " .
                    self::CONCOM_CREADO_POR . ")" .
                    " VALUES(:concom_descripcion,:fam_id,:concom_partida_global,:concom_partida_especifica,
                    CURRENT_TIMESTAMP(6),:concom_creado_por)";

                // Preparar la sentencia
                $sentencia = $pdo->prepare($comando);
                $sentencia->bindParam(":concom_descripcion", $concomDescripcion, PDO::PARAM_STR);
                $sentencia->bindParam(":fam_id", $famId, PDO::PARAM_STR);
                $sentencia->bindParam(":concom_partida_global", $concomPartidaGlobal, PDO::PARAM_INT);
                $sentencia->bindParam(":concom_partida_especifica", $concomPartidaEspecifica, PDO::PARAM_STR);
                $sentencia->bindParam(":concom_creado_por", $CreadoPor, PDO::PARAM_INT);
    ...
    private static function actualizar($datos, $id, $idUsuario)
    {
        if ($datos) {
            ...
            $concomPartidaGlobal = htmlspecialchars(strip_tags($datos->concom_partida_global));
            $concomPartidaEspecifica = htmlspecialchars(strip_tags($datos->concom_partida_especifica));
            $concomActivo = htmlspecialchars(strip_tags($datos->concom_activo));
            ...
            try {
                ...
                $consulta = "UPDATE " . self::NOMBRE_TABLA .
                    " SET " . self::CONCOM_DESCRIPCION . "= :concom_descripcion, " .
                    self::FAM_ID . "= :fam_id, " .
                    self::CONCOM_PARTIDA_GLOBAL . "= :concom_partida_global, " .
                    self::CONCOM_PARTIDA_ESPECIFICA . "= :concom_partida_especifica, " .
                    self::CONCOM_ACTIVO . "= :concom_activo, " .
                    self::CONCOM_MODIFICADO_EL . "= CURRENT_TIMESTAMP(6), " .
                    self::CONCOM_MODIFICADO_POR . "= :concom_modificado_por " .
                    " WHERE " . self::CONCOM_ID . "= :concom_id";
                ...
                $sentencia->bindParam(":concom_partida_global", $concomPartidaGlobal, PDO::PARAM_INT);
                $sentencia->bindParam(":concom_partida_especifica", $concomPartidaEspecifica, PDO::PARAM_STR);
                $sentencia->bindParam(":concom_modificado_por", $ModificadPor, PDO::PARAM_INT);
                $sentencia->bindParam(":concom_activo", $concomActivo, PDO::PARAM_INT);
                $sentencia->bindParam(":concom_id", $id, PDO::PARAM_STR);
                $sentencia->execute();
